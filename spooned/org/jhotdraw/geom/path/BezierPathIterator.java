 ...
 */
public class BezierPathIterator implements java.awt.geom.PathIterator {
    /**
     ...
     * The bezier path.
     */
    private org.jhotdraw.geom.path.BezierPath path;
     ...
     */
    public BezierPathIterator(org.jhotdraw.geom.path.BezierPath path) {
        this(path, null);
    }
     ...
     * @see BezierPath#getPathIterator
     */
    public BezierPathIterator(org.jhotdraw.geom.path.BezierPath path, java.awt.geom.AffineTransform at) {
        this.path = path;
        this.affine = at;
    }
    ...
    public int currentSegment(double[] coords) {
        ...
        if (index == path.size()) {
            ...
            if (path.size() > 1) {
                org.jhotdraw.geom.path.BezierPath.Node previous = path.nodes().get(path.size() - 1);
            ...
            org.jhotdraw.geom.path.BezierPath.Node current = path.nodes().get(index);
            coords[0] = current.x[0];
            coords[1] = current.y[0];
            numCoords = 1;
            ...
            org.jhotdraw.geom.path.BezierPath.Node current = path.nodes().get(index);
            org.jhotdraw.geom.path.BezierPath.Node previous = path.nodes().get(index - 1);
            if ((previous.mask & org.jhotdraw.geom.path.BezierPath.C2_MASK) == 0) {
                if ((current.mask & org.jhotdraw.geom.path.BezierPath.C1_MASK) == 0) {
                    numCoords = 1;
                    type = java.awt.geom.PathIterator.SEG_LINETO;
        ...
        if (affine != null) {
            ...
            java.lang.System.arraycopy(coords, 0, coords, 0, numCoords);
        }
        return type;
    }
}