/* @(#)MultiIcon.java

Copyright (c) 2008 The authors and contributors of JHotDraw.
You may not use, copy or modify this file, except in compliance with the
accompanying license terms.
 */
package org.jhotdraw.gui.plaf.palette;
/**
 * An icon which paints one out of multiple icons depending on the state of the component. MultiIcon
 * can lazily create the icons from a tiled image.
 */
public abstract class MultiIcon implements javax.swing.Icon {
    /**
     * The icons from which we choose from. This variable is null, if we are using a tiled image as
     * our base.
     */
    protected javax.swing.Icon[] icons;

    /**
     * Holds the icon pictures in a single image. This variable is used only until we create the icons
     * array. Then it is set to null.
     */
    private java.awt.Image tiledImage;

    /**
     * The number of icons in the tiledImage.
     */
    private int tileCount;

    /**
     * Whether the tiledImage needs to be tiled horizontaly or vertically to get the icons out of it.
     */
    private boolean isTiledHorizontaly;

    /**
     * Creates a new instance from an array of icons. All icons must have the same dimensions. If an
     * icon is null, an icon is derived for the state from the other icons.
     */
    public MultiIcon(javax.swing.Icon[] icons) {
        this.icons = icons.clone();
        generateMissingIcons();
    }

    /**
     * Creates a new instance from an array of images. All icons must have the same dimensions. If an
     * icon is null, an icon is derived for the state from the other icons.
     */
    public MultiIcon(java.awt.Image[] images) {
        this.icons = new javax.swing.Icon[images.length];
        for (int i = 0, n = icons.length; i < n; i++) {
            if (images[i] != null) {
                icons[i] = new javax.swing.ImageIcon(images[i]);
            }
        }
        generateMissingIcons();
    }

    public MultiIcon(java.awt.Image tiledImage, int tileCount, boolean isTiledHorizontaly) {
        this.tiledImage = tiledImage;
        this.tileCount = tileCount;
        this.isTiledHorizontaly = isTiledHorizontaly;
    }

    @java.lang.Override
    public int getIconHeight() {
        generateIconsFromTiledImage();
        return icons[0].getIconHeight();
    }

    @java.lang.Override
    public int getIconWidth() {
        generateIconsFromTiledImage();
        return icons[0].getIconWidth();
    }

    @java.lang.Override
    public void paintIcon(java.awt.Component c, java.awt.Graphics g, int x, int y) {
        generateIconsFromTiledImage();
        javax.swing.Icon icon = getIcon(c);
        if (icon != null) {
            icon.paintIcon(c, g, x, y);
        }
    }

    private void generateIconsFromTiledImage() {
        if (icons == null) {
            icons = new javax.swing.Icon[tileCount];
            java.awt.Image[] images = org.jhotdraw.util.Images.split(tiledImage, tileCount, isTiledHorizontaly);
            for (int i = 0, n = java.lang.Math.min(images.length, icons.length); i < n; i++) {
                if (images[i] != null) {
                    icons[i] = new javax.swing.ImageIcon(images[i]);
                }
            }
            generateMissingIcons();
            tiledImage = null;
        }
    }

    protected abstract javax.swing.Icon getIcon(java.awt.Component c);

    protected abstract void generateMissingIcons();
}